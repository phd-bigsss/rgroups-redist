...
Individual data from the International Social Survey Programme (ISSP) 2017, titled "Social Networks and Social Resources," is utilized [@issp_research_group_international_2019]. Established in 1984, the ISSP is a collaborative research project that conducts cross-national surveys on various topics related to social issues, attitudes, and values. Specifically, the ISSP 2017 survey includes questions on social ties and activities, evaluated through a position generator, as well as inquiries on social resources derived from network members. Additionally, respondents express their views on topics such as social trust and attitudes toward economic inequality. Initially, the dataset comprised 47,027 individual observations across 32 countries. However, Slovenia (SVN) was excluded from the analysis because the key dependent variable was not present in the questionnaire. Consequently, the present study comprises complete information on 31,191 individuals from 31 countries, representing a diverse range of economic, cultural, and institutional characteristics.
...
La presente investigación buscó responder dos preguntas centrales. Primero, ¿cuál es el rol de la class-based network segregation en las redistributive preferences? Segundo, ¿juega un rol la desigualdad económica en la relación entre class-based network segregation y las redistributive preferences?

Lo que la evidencia aquí presentada sugiere es que una mayor homogeneidad en términos de clase social tiene un efecto polarizante en las preferencias redistributivas. Así, este estudio ha demostrado que la homogeneidad de la red posee implicancias diferenciadas en las preferencias redistributivas según la posición de clase. Por un lado, para la clase trabajadora, poseer redes sociales más homogéneas se traduce en mayores preferencias redistributivas. En contraste, es posible evidenciar lo opuesto en la clase servicio, en donde redes más homogéneas se traducen en una disminución de las preferencias redistributivas. 

En relación a el rol de la desigualdad económica, dos alternativas han sido puestas bajo escrutinio.
Por un lado, nuestra expectativa era que la relación de nivel micro sería _amplificada_ por una mayor desigualdad economica, dado el incremento de la distancia social entre las clases sociales ($H_{2a}$). Por otro lado, hipotetizamos que la desigualdad economica puede _mitigar_ la relación entre segregación y preferenias redistributivias tiende a menguar en tanto aumenta la desigualdad económica ($H_{2b}$). En efecto, lo que encontramos es que en la medida que la desigualdad aumenta la relación a nivel micro se va debilitando, lo cual se traduce en una brecha de clase más acotada, principalmente a través de los cambios en las preferencias redistributivas en la clase servicio.

<!-- _conclusion_  -->

Por un lado, nuestra primera hipótesis se fundamenta en la claimed negative influence of the desigualdad económica en la cohesión social. 

Por otro lado, las investigaciones sobre desigualdad y preferencias redistributivas han establecido que las personas en la clase alta tiende a volverse más igualitarias en contextos mayor desigualdad económica [@edlund_democratic_2015;@sachweh_why_2019]. Además, evidencia reciente ha establecido en sociedades más desiguales las personas en posicions más aventajadas tienden a tener mayor diversidad socioeconómica en sus lazos sociales [@otero_differences_2023]. 

Pueden ser que sus chanches de participar incrementen sus posibilidades de diversidad al tener también mayor confianza - eso hace que también 


- network instrument, only a proxy of class positions, mainly the intermediate class positions 
- measurement of redistributive preferences
- causal claims and endogeneity problems  
...
c("Redistributive preferences","Class-based network homogeneity",
  "Network size","Social trust","Social class",
  "Household Income","Education in years",
  "Partner's Social Class","Labor status","Union membership",
  "Gender","Age in years","Religion","Has partner",
  "Income Inequality - Gini Index (z-score)",
  "GDP/capita (z-score)",
  "Size of the welfare state(z-score)")

sjlabelled::set_label(dfreg) <- varlab
# names(dfreg) <- varlab
...
texreg(digclas1_VW,stars = c(0.001, 0.01, 0.05, 0.1),
       caption = paste("(\\#tab:table2)","Multilevel models for Class-based network segregation and Redistributive Preferences"),
       custom.note = "Note: Models include sampling weights. Gender, age, marital status and religion are included as controls. Standard errors in parentheses. %stars",
       caption.above = T,leading.zero = T,
       custom.coef.map = ccoef,
       symbol = "+",scalebox = 0.60,
...
```{r fig4-socdist, fig.dim=c(10,5),fig.cap='Linear Predictions for Network Homogeneity on Redistributive Preferences by Social Class',warning=FALSE}
int_homo <- digclas1_VW_7
df_pred1 <- 
  predictions(int_homo, condition = c("homclass3_V_res","dclass3res_V"),
              newdata = datagrid(homclass3_V_res = seq(min(dfreg$homclass3_V_res), max(dfreg$homclass3_V_res), by= 10),
                                 dclass3res_V = levels(dfreg$dclass3res_V)))
# names(df_pred1)
df_pred1 %>% 
ggplot(aes(y =estimate,x=homclass3_V_res, fill=dclass3res_V, color=dclass3res_V,ymin=conf.low, ymax=conf.high)) +
...
```{r fig5-socdist, fig.dim=c(12,7),fig.cap='Three-way interaccion effects for Redistributive Preferences, Network segregation, Social class and Income Inequality'}
### Predicted values homo*class*GiniD ---------------------------------------
# ginid_max<- round(max(dfreg$gini_disp) ,2)
# ginid_min<- round(min(dfreg$gini_disp),2)
...
df_pred_giniD_gc <-
  predictions(
    homo_V_gini_full2,newdata = datagrid(
      ...
      wstate2=mean(dfreg$wstate2),
      loggdppercapita=mean(dfreg$loggdppercapita),
      homclass3_V_gc = seq(min(dfreg$homclass3_V_gc)+0.01, max(dfreg$homclass3_V_gc)+0.01, by=10),
      dclass3res_V=levels(dfreg$dclass3res_V)
    )
  )
