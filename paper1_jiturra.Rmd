...
# dfreg$wstate <- scale(scale(dfreg$rel_red) + scale(dfreg$gv_spen))
...
homo_V_gini_controls <- 
  lmer(egal~ 
         ...
         edyears_gc+Q03pcm_2_gc+Q03pcm_3_gc+Q03pcm_NA_gc+
         union_gc+workst_gc+
         gini_disp +loggdppercapita + wstate +
         (homclass3_V_gc+dclass3res_V|country2),data=dfreg,weights = WEIGHT)
...
digclas1_macro <- list(homo_V_gini,homo_V_gini_controls,homo_V_gini_full)
...
ccoef <- list(homclass3_V_gc="Class-based network homogeneity (CWC)",
              ...
              # gv_spen = "Government spending (percentage of GDP)",
               wstate="Size of Welfare State",
...
knitreg(digclas1_macro,stars = c(0.001, 0.01, 0.05, 0.1),symbol = "+",scalebox = 0.60,
       ...
       custom.coef.map = ccoef,
      # groups = list("Social Class (Ref.= Service Class)" = 2:3, "Homogeneity x Social Class"=8:9, "Homogeneity x Social Class x Income Inequality"=11:12),
       float.pos = "h!",
       use.packages = F)
```
...
df_pred_giniD_gc <-
  predictions(
    homo_V_gini_full,newdata = datagrid(
      gini_disp = c(ginid_min, ginid_mea, ginid_max),
      wstate=mean(dfreg$wstate),
