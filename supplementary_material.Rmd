...
pacman::p_load(dplyr,sjmisc,ggplot2,interplot,marginaleffects,sjlabelled,haven,stringr,ordinal,texreg,MLMusingR,lme4,influence.ME)
...
```{r tab-S1, results='asis', cache=TRUE}
...
texreg(digclas1_macro,stars = c(0.001, 0.01, 0.05, 0.1),symbol = "+",scalebox = 0.50,
       caption = "Multilevel models for Income inequality (Ratio 90/10), Network segregation and Redistributive Preferences",
       custom.note = "Note: Models include individual level controls centered within cluster (group mean). Standard errors in parentheses. %stars",
...
```{r tab-S2, results='asis', cache=TRUE}
set1 <-  RColorBrewer::brewer.pal(n = 8, name = "Set1")
options(ggplot2.discrete.fill = set1)
options(ggplot2.discrete.colour = set1)
ggplot2::theme_set(ggplot2::theme_bw())
...
texreg(digclas1_macro,stars = c(0.001, 0.01, 0.05, 0.1),symbol = "+",scalebox = 0.50,
       caption = "Multilevel models for Income inequality (Ratio 90/50), Network segregation and Redistributive Preferences",
       custom.note = "Note: Models include individual level controls centered within cluster (group mean). Standard errors in parentheses. %stars",
       caption.above = T,leading.zero = T,single.row = T,
       custom.coef.map = ccoef,
      groups = list("Social Class (Ref.= Service Class)" = 2:3,"Macro-level factors"=4:6, "Homogeneity*Social Class"=7:8, "Homogeneity * Social Class * Income Inequality"=10:11),
...
texreg(digclas1_Q05,stars = c(0.001, 0.01, 0.05, 0.1),single.row = F,include.ci = FALSE,custom.header = header,
       ...
       custom.note = "Note: Gender, age and marital status are included as controls. Standard errors in parentheses. %stars",
...
```{r fig-S2, fig.dim=c(15,5),fig.cap='Average Marginal Effects of Network homogeneity conditioned by Social class on Redistributive Preferences by Income Inequality Groups',warning=FALSE}
marginal <- bind_rows(
...
Several robustness checks were consistent with our main results. First, a (delete-one) jackknife procedure indicated that estimations for the segregation hypothesis are robust to outliers. Additonaly, Using DFbetas and Cook distance, there are influential cases. Subsequently I see that in some cases the value is altered, but these changes are not significant. Second, repeating the procedure with the models, including the cross-level interaction for the mitigation hypothesis, leads to relatively same results. 
...
results <- list()
countries<- levels(as.factor(dfreg$country2))

# Iterate over each country
for (i in countries){
  # Exclude the i-th country
  subset_data <-dfreg %>% dplyr::filter(country2 != i)
  # Refit the model without the i-th country
  model_i <- lmer(egal~homclass3_V_res+know_total+female+agenum+partner+edyears + Q03pcm + workst + (1 | country2) + dclass3res_V + homclass3_V_res:dclass3res_V, data = subset_data,weights = WEIGHT)
...
tidyresults <- list()
for (i in countries) {
  tidyresults[[i]] <- broom.mixed::tidy(results[[i]],conf.int=T)
}

jk<- bind_rows(tidyresults, .id = "country");jk$country <-paste("Excluding",jk$country) 
```

```{r fig-S1,fig.dim=c(10,10),fig.cap='Interaction of network homogeneity and social class on redistributive preferences. Excluding countries one by one.'}
jk %>% filter(term %in% c("homclass3_V_res",
                          "dclass3res_VIntermediate class (III+IV)","dclass3res_VWorking Class (V+VI+VII)",
                          "homclass3_V_res:dclass3res_VIntermediate class (III+IV)",
                          "homclass3_V_res:dclass3res_VWorking Class (V+VI+VII)")) %>% 
  ggplot(mapping = aes(y=estimate,x=term,ymin = conf.low, ymax = conf.high)) + 
  geom_point() +
  geom_errorbar(width = 0.2) +
  labs(x=NULL,y="Estimates") +
  scale_x_discrete(limits=rev(c("homclass3_V_res",
                          "dclass3res_VIntermediate class (III+IV)","dclass3res_VWorking Class (V+VI+VII)",
                          "homclass3_V_res:dclass3res_VIntermediate class (III+IV)",
                          "homclass3_V_res:dclass3res_VWorking Class (V+VI+VII)")),
                   labels=c("Hom:Working","Hom:Intermediate","Working Class","Intermediate Class","Homogeneity"))+
  geom_hline(yintercept = 0,color="red")+
  coord_flip() +
  facet_wrap(~country)
```
...
```{r eval=FALSE, fig.dim=c(5,5), include=FALSE}
library(influence.ME)
estex.m31 <- influence(digclas1_VW_7, "country2")
dfbetas(estex.m31) 
cutpoint<- 2/sqrt(31) # cut point

plot(estex.m31, which="cook",
     cutoff=4/31, sort=TRUE,
     xlab="Cooks Distance",
     ylab="Country")
# sigtest(estex.m31, test=-1.96)
```
...
for (i in countries){
  ...
  model_i <- lmer(egal~ 
         ...
         dclass3res_V*
         gini_disp + 
         (homclass3_V_gc+dclass3res_V|country2),data=subset_data,weights = WEIGHT)
...
# texreg(results[1:10])
# knitreg(results[11:20])
# knitreg(results[21:31])
...
ccoef <- list(homclass3_V_res="ISEI-based network homogeneity/10",
              know_total="Network size",
              # socialtrust="Social Trust",
              # "dclass3res_VIntermediate class (III+IV)"="Intermediate Class",
              # "dclass3res_VWorking Class (V+VI+VII)"="Working Class",
              ...
              "homclass3_V_res:dclass3res_V" = "Homogeneity*ISEI")


texreg(digclas1_VW,stars = c(0.001, 0.01, 0.05, 0.1),
       caption = paste("(\\#tab:table2)","Multilevel models for ISEI-based network homogeneity and Redistributive Preferences"),
       custom.note = "Note: Gender, age, marital status and religion are included as controls. Standard errors in parentheses. %stars",
       caption.above = T,leading.zero = T,
       custom.coef.map = ccoef,
...
```{r fig-S3, fig.dim=c(10,5),fig.cap='Average Marginal Effects of ISEI-based network homogeneity conditioned by ISEI on Redistributive Preferences',warning=FALSE}
...
```{r fig-S4, fig.dim=c(10,5),fig.cap='Linear Predictions for ISEI-based network homogeneity on Redistributive Preferences by ISEI',warning=FALSE}
int_homo <- digclas1_VW_7
df_pred1 <- 
...
ggplot(aes(y =estimate,x=homclass3_V_res, fill=dclass3res_V, color=dclass3res_V,ymin=conf.low, ymax=conf.high)) +
  ...
  labs(caption = paste0("N=",nobs(int_homo),", Countries=",lme4::ngrps(int_homo)),
       x="ISEI-based network homogeneity",y="Redistributive preferences") +
  theme(legend.position = "bottom", legend.title = element_blank())
```

\pagebreak

```{r tab-S5, results='asis', cache=TRUE}
set1 <-  RColorBrewer::brewer.pal(n = 8, name = "Set1")
options(ggplot2.discrete.fill = set1)
options(ggplot2.discrete.colour = set1)
ggplot2::theme_set(ggplot2::theme_bw())
...
df1$dclass3spo_V <- car::recode(df1$dclass3spo_V,"NA='No information (Missing, No partner)'",levels = c("Service Class (I+II)","Intermediate class (III+IV)","Working Class (V+VI+VII)","No information (Missing, No partner)"))
dummy_df <- df1 %>% dplyr::select(id,redist,prefineq,
  egal = egal2,dclass3res_V,
  # dclass3spo_V,
...
df1$isei08sp <- car::recode(df1$isei08sp,"NA=0")
dfreg <- df1 %>% dplyr::select(id,redist,prefineq,
  egal = egal2,
  # starts_with("class3"),
  # starts_with("digclass3"),
  # starts_with("dclass3"),
...
ccoef <- list(homclass3_V_gc="ISEI-based network homogeneity (CWC)",
              "dclass3res_V"="ISEI ",
              "gini_disp" = "Income inequality (Gini index)",
              loggdppercapita = "GDP/capita",
              wstate2="Size of the welfare state",
              "homclass3_V_gc:dclass3res_V" = "Homogeneity*ISEI",
              "homclass3_V_gc:gini_disp"= "Homogeneity*Income Inequality",
              "dclass3res_V:gini_disp" ="ISEI*Inequality",
              "homclass3_V_gc:dclass3res_V:gini_disp" = "Homogeneity*Working Class*Income Inequality")

texreg(digclas1_macro,stars = c(0.001, 0.01, 0.05, 0.1),symbol = "+",scalebox = 0.60,
       caption = "Multilevel models for Income Inequality, Network homogeneity and Redistributive Preferences",
       custom.note = "Note: Models include individual level controls centered within cluster (group mean). Standard errors in parentheses. %stars",
       caption.above = T,leading.zero = T,single.row = T,
       custom.coef.map = ccoef,
      ...
      custom.gof.names = c(NA,NA,"Num. groups",
                           ...
                           "Cov: Group Homogeneity, ISEI",
                           "Var: Residual"),
       use.packages = F)
```

```{r fig-S5, fig.dim=c(12,7),fig.cap='Three-way interaccion effects for Redistributive Preferences, ISEI-based network homogeneity, ISEI and Income Inequality'}
...
df_pred_giniD_gc %>% 
  ...
  labs(y = "Preferences for Redistribution",
       x="ISEI-based network homogeneity (CWC)",
