...
ggplot2::theme_update(text=element_text(size=9,  family="serif"))
...
# Predicted values 
predval1 <- 
plot_predictions(giniD_int, condition = list("homclass",gini_disp="threenum"),gray = T) +
...
# Predicted values 
predval2 <- 
plot_predictions(giniM_int, condition = list("homclass",gini_mkt="threenum"),gray = T)+
...
# Predicted values 
predval1 <- 
plot_predictions(giniD_int, condition = list("isolation",gini_disp="threenum"),gray = T) +
  labs(caption = paste0("N=",nobs(giniD_int),", Countries=",lme4::ngrps(giniD_int)),
       linetype='Gini (Disposable)') +
  xlab("Perceived Isolation")+
  ylab("Redistributive preferences") +
...
# Predicted values 
predval2 <- 
plot_predictions(giniM_int, condition = list("isolation",gini_mkt="threenum"),gray = T)+
  labs(caption = paste0("N=",nobs(giniM_int),", Countries=",lme4::ngrps(giniM_int)),
       linetype='Gini (Market)') +
  xlab("Perceived Isolation")+
  ylab("Redistributive preferences") +
...
```{r mod-other-macro,fig.height=3,fig.width=6,out.width='70%',results='asis'}
if (!require("pacman")) install.packages("pacman")
pacman::p_load(dplyr,sjmisc,ggplot2,interplot,sjPlot,sjlabelled,haven,stringr,gridExtra)  
rm(list=ls())
load(here::here("input/data/proc/study1_country.RData"));df1 <- df2
...
int_palma <- lmer(redist~class3+ isolation+homclass*palmawd+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna + (1+homclass|country2),data=dfreg)
int_pc10  <- lmer(redist~class3+ isolation+homclass*pc10_2017+edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna +(1+homclass|country2),data=dfreg)
int_pc50  <- lmer(redist~class3+ isolation+homclass*pc50_2017+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna +(1+homclass|country2),data=dfreg)
int_r1090 <- lmer(redist~class3+ isolation+homclass*ratio10_50+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna +(1+homclass|country2),data=dfreg)
...
models_m <- list(int_palma,int_pc10,int_pc50,int_r1090,int_indivi,int_govspe)
...
list(homclass = "Class-based network homogeneity",
  ...
  pc10_2017             = "Share National Income - top 10%",
  "homclass:pc10_2017"    = "Homogeneity x top 10",
  pc50_2017             = "Share National Income -  bottom 50%",
  "homclass:pc50_2017"    = "Homogeneity x bottom 50",
  ratio10_50            = "Ratio (90/50)",
  "homclass:ratio10_50"   = "Homogeneity x Ratio (90/50)",
...
texreg::knitreg(models_m,custom.coef.map = customcoef,caption.above = T,caption = cap1)
```
...
p5<- interplot::interplot(m = int_pc50,var1 = "homclass",var2="pc50_2017") +
  xlab("Share National Income - bottom 50%") + geom_point()+ geom_hline(yintercept = 0) +
...
predval8 <- 
plot_predictions(int_indivi, condition = list("homclass",individualism="threenum"),gray = T) +
  labs(caption = paste0("N=",nobs(int_indivi),", Countries=",lme4::ngrps(int_indivi)),
...
predval9 <- 
plot_predictions(int_govspe, condition = list("homclass",gv_spen=range),gray = T) +
  labs(caption = paste0("N=",nobs(int_govspe),", Countries=",lme4::ngrps(int_govspe)),
       linetype='Gov.Spend') +
  xlab("Class-based homogeneity")+
  ylab("Redistributive preferences") +
