...
old<- ggplot2::theme_set(theme_minimal())
ggplot2::theme_set(old)
ggplot2::theme_update(text=element_text(size=8,  family="serif"))
```
#  Introduction
...
giniOT_gdp<- lmer(redist~class3+ homclass+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+ giniot + logrgdpna+ (1|country2),data=dfreg)
giniOT_rs<- lmer(redist~class3+ homclass+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+ giniot + logrgdpna+ (1+homclass|country2),data=dfreg)
# anova(giniOT_gdp,giniOT_rs)
giniOT_int <- lmer(redist~class3+ homclass*giniot+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+ giniot + logrgdpna+ (1+homclass|country2),data=dfreg)
...
# Predicted values  
pred1<- sjPlot::plot_model(model = giniD_int,type = "int", mdrt.values = "meansd")
pred_data <- data.frame(pred1$data)
pred_data <- pred_data %>% arrange(pred_data$group)
...
# Predicted values 
pred1<- sjPlot::plot_model(giniM_int,type = "int", mdrt.values = "meansd")
pred_data <- data.frame(pred1$data)
pred_data <- pred_data %>% arrange(pred_data$group)
...
```{r crossginiD, fig.height=4,fig.width=9.3,fig.cap=c("Cross-level interaction for Network homogeneity and Gini (Disposable)"),out.width='50%'}
grid.arrange(p1,predval1,nrow = 1
             # top = "Network homogeneity × Gini (Disposable)" 
             )
```

```{r crossginiM, fig.height=4,fig.width=9.3,fig.cap=c("Cross-level interaction for Network homogeneity and Gini (Market)"),out.width='50%'}
...
```{r econineq,fig.height=4,fig.width=8,fig.cap=c("Cross-level interaction for Network homogeneity and Economic Inequality"),out.width='50%'}
if (!require("pacman")) install.packages("pacman")
pacman::p_load(dplyr,sjmisc,ggplot2,interplot,sjPlot,sjlabelled,haven,stringr)  
rm(list=ls())
load(here::here("input/data/proc/study1_country.RData"));df1 <- df2
...
int_palma <- lmer(redist~class3+ homclass*palmawd+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna + (1+homclass|country2),data=dfreg)
int_pc10  <- lmer(redist~class3+ homclass*pc10_2017+edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna +(1+homclass|country2),data=dfreg)
int_pc50  <- lmer(redist~class3+ homclass*pc50_2017+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna +(1+homclass|country2),data=dfreg)
int_r1090 <- lmer(redist~class3+ homclass*ratio10_50+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna +(1+homclass|country2),data=dfreg)
int_giniot<- lmer(redist~class3+ homclass*giniot+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna + (1+homclass|country2),data=dfreg)

int_giniot<- lmer(redist~class3+ homclass*giniot+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna + (1+homclass|country2),data=dfreg)

int_indivi<- lmer(redist~class3+ homclass*individualism+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna + (1+homclass|country2),data=dfreg)

int_govspe<- lmer(redist~class3+ homclass*gv_spen+ edyears+ Q03pcm+ labst1+ union+ female+agenum +age2+logrgdpna + (1+homclass|country2),data=dfreg)


models_m <- list(int_palma,int_pc10,int_pc50,int_r1090,int_giniot,int_indivi,int_govspe)
texreg::knitreg(models_m)

library(gridExtra)

p3<- interplot::interplot(m = int_palma,var1 = "homclass",var2="palmawd") +
...
p4<- interplot::interplot(m = int_pc10,var1 = "homclass",var2="pc10_2017") +
...
p5<- interplot::interplot(m = int_pc50,var1 = "homclass",var2="pc50_2017") +
...
```{r, fig.height=3.5,fig.width=6.5,fig.cap=c("Cross-level interaction for Network homogeneity Gov. Spending and Individualism"),out.width='50%'}
...
- Homogeneity x Economic Inequality
- Perceived Isolation
...
```{r crossginiD2, fig.height=4,fig.width=9.3,fig.cap=c("Cross-level interaction forPerceived Isolation and Gini (Disposable)"),out.width='50%'}
grid.arrange(p1,predval1,nrow = 1
