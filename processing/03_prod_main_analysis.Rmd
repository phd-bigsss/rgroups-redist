...
ggsave(plot = last_plot(),filename = "figure002.png",device = "tiff",
       path = here::here("output/images"),width = 2,height = 1,units = "cm",scale = 15,dpi = 500)
```
...
ggplot()+
  ...
  theme(legend.position = "bottom",
        ...
        axis.text.x=element_text(angle=45,hjust = 1,size=8),
        legend.title = element_blank())  
ggsave(plot = last_plot(),filename = "figure003.png",device = "tiff",
       path = here::here("output/images"),width = 2,height = 1,units = "cm",scale = 15,dpi = 500)
```
...
ggsave(plot = arrangeGrob(fig_homo_gini,fig_egal_homo,
 ...
 filename = "figure004.png",device = "tiff",
 path = here::here("output/images"),width = 1,height = 1,units = "cm",scale = 26,dpi = 500)
```
...
digclas1_VW_0<- lmer(egal~1 + dclass3res_V + female+agenum+know_total+(1|country2),data=dfreg,weights = WEIGHT)
...
digclas1_VW_3 <- update(digclas1_VW_0, . ~ . + dclass3res_V)
# digclas1_VW_5 <- update(digclas1_VW_3, . ~ . + edyears+ Q03pcm+workst)

digclas1_VW_7 <- update(digclas1_VW_0, . ~ . +dclass3res_V*homclass3_V_res+know_total)
digclas1_VW <- list(digclas1_VW_0,
                    # digclas1_VW_1,
                    # digclas1_VW_3,
                    # digclas1_VW_4,
                    #digclas1_VW_5,
...
htmlreg(digclas1_VW,stars = c(0.001, 0.01, 0.05, 0.1),single.row = T,symbol = ".",
       caption = paste("Table 1: Multilevel models for social class, network homogeneity and redistributive preferences"),
       custom.note = "Note: Standard errors in parentheses. %stars",
       threeparttable = T,
       caption.above = T,leading.zero = T,
       custom.coef.map = ccoef,
       groups =
         list("Social Class (Ref.= Service Class)" = 2:3,
...
ggsave(plot = last_plot(),filename = "figure005.png",device = "tiff",
       path = here::here("output/images"),width = 2,height = 1,units = "cm",scale = 15,dpi = 500)
```
...
ccoef <- list("dclass3res_VIntermediate class (III+IV)"="Intermediate Class",
              "dclass3res_VWorking Class (V+VI+VII)"="Working Class",
              homclass3_V_gc="Class-based network homogeneity",
...
htmlreg(digclas1_macro,stars = c(0.001, 0.01, 0.05, 0.1),scalebox = 0.7,symbol = ".",
       caption = paste("Table 2: Multilevel models for income inequality, social class, network homogeneity and redistributive preferences"),
      ...
      groups = list("Social Class (Ref.= Service Class)" = 1:2,
                    "Social Class × Homogeneity"=4:5,
                    "Class × Income Inequality" = 8:9 ,
                    "Social Class × Homogeneity × Income Inequality"=10:11),
...
ggsave(plot = last_plot(),filename = "figure006.png",device = "tiff",
       path = here::here("output/images"),width = 2,height = 1,units = "cm",scale = 15,dpi = 500)
```
...
dfreg <- df1 %>% dplyr::select(
  ...
  filter(country2 != "SVN") 
dfreg$egal <- as.numeric(scale(dfreg$egal))
dfreg_1 <- na.omit(dfreg %>% dplyr::select(prefineq,redist,
  egal,
  dclass3res_V,
  homclass3_V_res,
  know_total,
...
## Model -  EGP3 - DIGCLASS - V to Working -----------------------------$ 
digclas1_VW_0<- lmer(egal~1 + (1|country2),data=dfreg_1,weights = WEIGHT)
digclas1_VW_2 <- update(digclas1_VW_0, . ~ . + dclass3res_V)
digclas1_VW_3 <- update(digclas1_VW_2, . ~ . + dclass3res_V+homclass3_V_res+female+agenum+know_total)
digclas1_VW_7 <- update(digclas1_VW_3, . ~ . +dclass3res_V*homclass3_V_res)
#Include additional controls
digclas1_VW_8 <- update(digclas1_VW_7, . ~ . + edyears+Q03pcm+partner+workst,data=na.omit(dfreg))
digclas1_VW <- list(digclas1_VW_2,digclas1_VW_3,digclas1_VW_7,digclas1_VW_8)
...
htmlreg(digclas1_VW,stars = c(0.001, 0.01, 0.05, 0.1),single.row = T,symbol = ".",
      ...
      groups = list("Social Class (Ref.= Service Class)" = 1:2,
                    "Social Class × Homogeneity"=5:6,
                    "Household Income (Ref.= Tertile I)"= 8:10
                    ),
