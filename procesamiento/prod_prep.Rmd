...
pacman::p_load(dplyr,sjmisc,GGally,sjlabelled,tidyverse)
...
```{r income-respondent}
income_country <- names(df1 %>% select(dplyr::ends_with("_RINC")))
# lapply(df1[,income_country], table)
for (i in income_country) {
  df1[[i]] <- car::recode(var = df1[[i]],"999990:99999999=NA")
  df1[[i]] <- as.numeric(df1[[i]])
...
rate90_10_wb<- rate90_10_wb %>% filter(ccode %in% countries$ccode)
...
summary(wiid$gini)
gini_wiid <- wiid %>% select(c3,year,gini) %>% filter(year %in% c(2010:2020))
summary(gini_wiid)
...
summary(gini_wiid$giniwd)
# gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2019,no = gini_wiid$giniwd)
# summary(gini_wiid$giniwd)
# gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2020,no = gini_wiid$giniwd)
# summary(gini_wiid$giniwd)
gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2016,no = gini_wiid$giniwd)
summary(gini_wiid$giniwd)
# gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2015,no = gini_wiid$giniwd)
# summary(gini_wiid$giniwd)
# gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2014,no = gini_wiid$giniwd)
# summary(gini_wiid$giniwd)
# gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2013,no = gini_wiid$giniwd)
# summary(gini_wiid$giniwd)
# gini_wiid$giniwd <- ifelse(is.na(gini_wiid$giniwd),gini_wiid$gini_2012,no = gini_wiid$giniwd)
# summary(gini_wiid$giniwd)

country_data2 <- 
country_data %>% left_join(gini_wiid[,c("ccode","giniwd","c3")],by="ccode")
```

```{r}
df1$ccode <- as.numeric(df1$country)
df2 <- 
df1 %>% left_join(country_data2[,c("ccode","c3","giniwb","giniwd")],by = "ccode")
```
...
```{r}
df1 <- df2
save(df2,file = here::here("input/data/proc/study1.RData"))
```
