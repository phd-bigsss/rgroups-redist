...
# relig <- haven::read_dta(here::here("input/data/original/ZA7570_v2-1-0.dta"))
# religTR <- relig %>% filter(country==792)
...
df1 <- df1 %>% 
  mutate(oesch8spo = isco88_to_oesch(isco88spo, self_employed_spo, n_employees_spo, n_classes = 16,to_factor = T,label=T),
...
# EI homophily index
df1$ingroup_res <- NA 
df1 %>% select(ddigclass3res,n_high,n_middle,n_low,ingroup_res)
