...
haven::write_dta(data = df1[c("id","isco88spo","sempspo","supvisspo")],
                 path = "input/data/proc/ocup_spouse.dta")
df1$id <- NULL

# Create a temporary variable `tmpisco88` and assign it the value of `isco88`
df1$tmpisco88 <- df1$isco88

# Replace values in `tmpdf1$isco88` based on specified conditions
df1$tempisco <- ifelse((df1$isco88spo >= 6100 & df1$isco88spo <= 6133) & df1$supvis >= 1 & !is.na(df1$supvis), 1311, df1$isco88spo)
df1$tempisco <- ifelse((df1$isco88spo >= 9200 & df1$isco88spo <= 9213) & df1$supvis > 1 & !is.na(df1$supvis), 6132, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1311 & df1$supvis > 10 & !is.na(df1$supvis), 1221, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1312 & df1$supvis > 10 & !is.na(df1$supvis), 1222, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1313 & df1$supvis > 10 & !is.na(df1$supvis), 1223, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1314 & df1$supvis > 10 & !is.na(df1$supvis), 1224, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1315 & df1$supvis > 10 & !is.na(df1$supvis), 1225, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1316 & df1$supvis > 10 & !is.na(df1$supvis), 1226, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1317 & df1$supvis > 10 & !is.na(df1$supvis), 1227, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1318 & df1$supvis > 10 & !is.na(df1$supvis), 1228, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1319 & df1$supvis > 10 & !is.na(df1$supvis), 1229, df1$isco88spo)
df1$tempisco <- ifelse((df1$isco88spo == 1300 | df1$isco88spo == 1310) & df1$supvis > 10 & !is.na(df1$supvis), 1220, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1221 & (df1$supvis >= 1 & df1$supvis <= 10), 1311, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1222 & (df1$supvis >= 1 & df1$supvis <= 10), 1312, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1223 & (df1$supvis >= 1 & df1$supvis <= 10), 1313, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1224 & (df1$supvis >= 1 & df1$supvis <= 10), 1314, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1225 & (df1$supvis >= 1 & df1$supvis <= 10), 1315, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1226 & (df1$supvis >= 1 & df1$supvis <= 10), 1316, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1227 & (df1$supvis >= 1 & df1$supvis <= 10), 1317, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1228 & (df1$supvis >= 1 & df1$supvis <= 10), 1318, df1$isco88spo)
df1$tempisco <- ifelse(df1$isco88spo == 1229 & (df1$supvis >= 1 & df1$supvis <= 10), 1319, df1$isco88spo)
df1$tempisco <- ifelse((df1$isco88spo == 1200 | df1$isco88spo == 1210 | df1$isco88spo == 1220) & (df1$supvis >= 1 & df1$supvis <= 10), 1310, df1$isco88spo)
df1$tempisco <- ifelse((df1$isco88spo == 1220 | (df1$isco88spo >= 1222 & df1$isco88spo <= 1229)) & df1$semp == 1 & df1$supvis >= 11 & !is.na(df1$supvis), 1210, df1$isco88spo)
...
df1$egp10spo <- df1$egp10_b
df1$egp10_b <- NULL
sjmisc::frq(df1$egp10spo)

# total N=44492 valid N=21626 mean=4.87 sd=3.29
# Value |                Label |     N | Raw % | Valid % | Cum. %
# ---------------------------------------------------------------
#     1 |   higher controllers |  3321 |  7.46 |   15.36 |  15.36
#     2 |       lo controllers |  4658 | 10.47 |   21.54 |  36.90
#     3 |    routine nonmanual |  3303 |  7.42 |   15.27 |  52.17
#     4 |       sempl with emp |   473 |  1.06 |    2.19 |  54.36
#     5 |   sempl without empl |  1638 |  3.68 |    7.57 |  61.93
#     7 |    manual supervisor |     1 |  0.00 |    0.00 |  61.93
#     8 |       skilled manual |  3055 |  6.87 |   14.13 |  76.06
#     9 | semi-unskilld manual |  4027 |  9.05 |   18.62 |  94.68
#    10 |           farm labor |   677 |  1.52 |    3.13 |  97.81
#    11 |        selfempl farm |   473 |  1.06 |    2.19 | 100.00
#  <NA> |                 <NA> | 22866 | 51.39 |    <NA> |   <NA>

# 
#           class10spo |      Freq.     Percent        Cum.
# ---------------------+-----------------------------------
#   higher controllers |      2,481       11.47       11.47
#       lo controllers |      5,505       25.46       36.93
#    routine nonmanual |      3,113       14.39       51.32
#       sempl with emp |        895        4.14       55.46
#   sempl without empl |      1,606        7.43       62.89
#    manual supervisor |        410        1.90       64.78
#       skilled manual |      2,373       10.97       75.76
# semi-unskilld manual |      4,091       18.92       94.67
#           farm labor |        433        2.00       96.68
#        selfempl farm |        719        3.32      100.00
# ---------------------+-----------------------------------
#                Total |     21,626      100.00
