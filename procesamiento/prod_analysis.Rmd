...
dfreg$homclass_bi <- ifelse(dfreg$homclass>0.5,1,0)
frq(dfreg$homclass_bi)
...
full_m <- lmer(redist~1 +female+agenum + labst + union+ class3 +ednum+homclass +  Q05pcm +  gini_disp + logrgdpna + (1|country),data=dfreg)
...
full_int4 <- lmer(redist~1 +female+agenum + labst + union+ class3 + homclass*gini_disp+ednum + Q05pcm  +logrgdpna + (1+homclass|country),data=dfreg)
...
models_m <- list(full_int4)
texreg::knitreg(models_m)

interplot::interplot(m = full_int4,var1 = "homclass",var2="gini_disp") +
  geom_point()+
  ylab("Class-based homogeneity coefficient")+
...
```{r strong-ties-macro}
# Interactions segregation x inequality
full_int4 <- lmer(redist~1 +female+agenum + labst + union+ class3 + homclass_s*gini_disp+ednum + Q05pcm +logrgdpna + (1+homclass_s|country),data=dfreg)
# full_int5 <- lmer(redist~1 +female+agenum + labst + union+ class3 + homclass_s+pi_nteg*gini_disp+ednum + Q05pcm + tr_inte +logrgdpna + (1+pi_nteg|country),data=dfreg)

models_m <- list(full_int4)
texreg::knitreg(models_m)

interplot::interplot(m = full_int4,var1 = "homclass_s",var2="gini_disp") +
  geom_point()+
  ylab("Class-based homogeneity (s) coefficient")+
  xlab("Economic Inequality")+
  ggtitle("Government's responsibility: reduce the differences in income between people with high incomes and those with low incomes") +
...
ggsave(plot = last_plot(),filename = "mar_strong-ties_gini.png",device = "png",
       path = here::here("output"),width = 2,height = 1,units = "cm",scale = 10)
...
```{r weak-ties-macro}
# Interactions segregation x inequality
full_int4 <- lmer(redist~1 +female+agenum + labst + union+ class3 + homclass_w*gini_disp+ednum + Q05pcm +logrgdpna + (1+homclass_w|country),data=dfreg)
# full_int5 <- lmer(redist~1 +female+agenum + labst + union+ class3 + homclass_s+pi_nteg*gini_disp+ednum + Q05pcm + tr_inte +logrgdpna + (1+pi_nteg|country),data=dfreg)

models_m <- list(full_int4)
texreg::knitreg(models_m)

interplot::interplot(m = full_int4,var1 = "homclass_w",var2="gini_disp") +
  geom_point()+
  ylab("Class-based homogeneity (w) coefficient")+
  xlab("Economic Inequality")+
  ggtitle("Government's responsibility: reduce the differences in income between people with high incomes and those with low incomes") +
...
ggsave(plot = last_plot(),filename = "mar_weak-ties_gini.png",device = "png",
       path = here::here("output"),width = 2,height = 1,units = "cm",scale = 10)
