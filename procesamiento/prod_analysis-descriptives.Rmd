...
```{r class5}
if (!require("pacman")) install.packages("pacman") # instalar pacman
pacman::p_load(dplyr,sjmisc,GGally,ggplot2,haven)
load(here::here("input/data/proc/study1.RData"))
df1$country2 <- countrycode::countrycode(sourcevar = df1$country,origin = "iso3n",destination = "country.name")
percentage_data <- df1 %>%
...
cormat1 <- 
df1 %>% 
    select('redist',homclass,homclass_s,homclass_w,,class3,) %>% 
  na.omit() %>% 
  polycor::hetcor(parallel = T)
...
ggplot()+
 ...
 # geom_point(aes(x = country3,y = egal2_c)) +    
  theme_bw() +
  theme(legend.position = "bottom",
