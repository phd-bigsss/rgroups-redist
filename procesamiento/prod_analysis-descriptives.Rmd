...
plot(
  ...
  labs(title = "Redistributive preferences",caption = "\u03b1 = 0.70")

ggsave(plot = last_plot(),filename = "redist_index.png",device = "png",
       path = here::here("output/images"),width = 1.5,height = 0.75,units = "cm",scale = 18)
...
load(here::here("input/data/proc/study1_country.RData"));df1 <- df2
dfreg <- df1 %>% dplyr::select(egal2,redist,
                        class3=class3,
                        homclass,
                        Q03pcm,
                        edyears,
...
ggplot()+
  ...
  labs(title = "Redistributive preferences (index) and social class by country",x=NULL,y="Redistributive Preferences") +
  theme_bw() +
...
ggplot()+
  ...
  scale_shape_manual(values = c(6, 15, 16)) +
  scale_color_manual(values = c("blue", "#E7B800", "red")) +
  labs(title = "Class-based network homogeneity and social class by country",x=NULL,y="Redistributive Preferences") +
  theme_bw() +
  theme(legend.position = "bottom",
        axis.text.x=element_text(angle=45,hjust = 1),
        legend.title = element_blank()) 
...
macro_data <- df2 %>%
  ...
  summarise_at(c("redist","egal2","homclass","pi_isola","tr_courts","tr_inte","gini_disp","gini_mkt",
...
- the original one is the best oneó
